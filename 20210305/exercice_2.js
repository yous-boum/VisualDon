...
const villes = [
  ...
  const nomsDesVilles = villes.map((v)=>v.nom) // <- utilisez une la méthode adaptée sur "villes"
  ...
  */
  ...
  const villesDePlusDe30000Habitants = villes.filter(v=>v.population>30000)
...
//  
  ...
  const sommeHabitants = villes.reduce((resultat, depart)=> resultat +depart.population,0)
